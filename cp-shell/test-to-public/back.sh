...
while IFS= read -r backup_dir || [[ -n "$backup_dir" ]]; do
    ...
    # Step 2: 当前 public 目录是否存在
    if [ -d "$public_dir" ]; then
        timestamp=$(date "+%Y%m%d%H%M%S")
        backup_public_dir="$parent_dir/public_back_$timestamp"
        log "发现当前 public 目录，正在备份为：$backup_public_dir"
        mv "$public_dir" "$backup_public_dir"
        log "已备份当前 public 目录为：$backup_public_dir"
    else
        log "未找到当前 public 目录，无需备份"
    fi

    # Step 3: 将备份目录更名为 public
    mv "$backup_dir" "$public_dir"
    if [ $? -eq 0 ]; then
        log "成功将 $backup_dir 恢复为 $public_dir"
    else
        log "错误：无法将 $backup_dir 恢复为 $public_dir"
    fi
